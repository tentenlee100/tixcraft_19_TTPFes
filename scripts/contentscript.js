function updateChechCodeView() {
  ...
  const newName =
    "TicketForm[agree]" + html.substring(index + findString.length, endIndex);
...
function updateOrderView() {
  ...
  $(".order_item").each((index, item) => {
    ...
    hasTTPOrder = $(item).text().indexOf("嗚吼嗚吼吼") > -1;
  });
  ...
  $(".order_item").each((index, item) => {
    ...
    $(item)
      ...
      .each((index2, item2) => {
        if (index2 === 0) {
          ...
          orderInfo["round"] = text.substring(
            findIndex + "【第".length,
            endIndex
          );
        } else if (index2 === 1) {
    ...
    if (
      !(orderInfo.hasOwnProperty("round") && orderInfo.hasOwnProperty("name"))
    ) {
      return;
    }
    $(item)
  ...
  const table1Array = allOrder
    .reduce((sum, obj, nowIndex, array) => {
  ...
  table2Array.forEach((element) => {
    ...
    if (element.round == "1") {
      table2 += "03/13 11:00~12:00  (11:40排隊截止)";
    } else if (element.round == "2") {
      table2 += "03/13 12:30~13:30  (13:10排隊截止)";
    } else if (element.round == "3") {
      table2 += "03/13 14:00~15:00  (14:40排隊截止)";
    } else if (element.round == "4") {
      table2 += "03/13 15:30~16:30  (16:10排隊截止)";
    } else if (element.round == "5") {
      table2 += "03/13 17:00~18:00  (17:40排隊截止)";
    } else if (element.round == "6") {
      table2 += "03/13 18:30~19:30  (19:10排隊截止)";
    } else if (element.round == "7") {
      table2 += "03/14 11:00~12:00　(11:40排隊截止)";
    } else if (element.round == "8") {
      table2 += "03/14 12:30~13:30　(13:10排隊截止)";
    } else if (element.round == "9") {
      table2 += "03/14 14:00~15:00　(14:40排隊截止))";
    } else if (element.round == "10") {
      table2 += "03/14 15:30~16:30　(16:10排隊截止)";
    } else if (element.round == "11") {
      table2 += "03/14 17:00~18:00　(17:40排隊截止)";
    } else if (element.round == "12") {
      table2 += "03/14 18:30~19:30　(19:10排隊截止)";
    }
  ...
  table3Array.forEach((element) => {
    ...
    <td>${element.sum}</td>
    <td>${(
      (parseFloat(element.sum) / parseFloat(sum)) *
      100.0
    ).toLocaleString("en", { maximumFractionDigits: 2 })} %</td>
  </tr>
    `;
  });
  table3 += `</table>`;

  const table4Array = table1Array
    .reduce((sum, obj) => {
  ...
  $("#downloadExcelButton").click(function () {
    ...
    var data = [["成員名稱", "數量", "佔比"]];
    data = data.concat(
      table3Array.map((item) => {
        return [
          item.name,
          item.sum,
          (
            (parseFloat(item.sum) / parseFloat(sum)) *
            100.0
    ...
    XLSX.writeFile(
      wb,
      "TTP 4th 握手會報表_" + moment().format("YYYYMMDDHHmm") + ".xlsx"
    );
  });
}
...
function updateInputButtonToAButton() {
  $(".gamelist")
    .find(".row")
    .append(
      `<div class="col-xs-12" style="margin-top: 12px; color:red;"><p style="float:right; ">立即訂購 按鈕可以按ctrl+右鍵 or 滑鼠中鍵開啟新分頁 </p></div>`
    );
...
function main() {
  ...
  if (url.indexOf("/activity/detail/21_TTP4th") > -1) {
    //主頁，將input改成 a連結
    updateMasterView();
  }
  if (url.indexOf("/activity/game/21_TTP4th") > -1) {
    //場次列表頁，將input改成 a連結
    updateInputButtonToAButton();
  }
  if (url.indexOf("/ticket/verify/21_TTP4th/") > -1) {
    //輸入握手券頁面，強制將input轉大寫
    updateEnterCodeView();
  }
}
